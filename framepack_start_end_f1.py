        ...
        try:
            ...
            for name, model in _models.items():
                if hasattr(model, 'to'):
                    try:
                        model.to('cpu')
                        print(f"[FramePack F1] 已將 {name} 移回 CPU")
                    except Exception as e:
                        print(f"[FramePack F1] {name} 移回 CPU 失敗: {e}")
            ...
            print("[FramePack F1] 模型已卸載，顯存已釋放")
            
        except Exception as e:
            print(f"[FramePack F1] 卸載過程中發生錯誤: {e}")
    else:
        print("[FramePack F1] 沒有載入的模型需要卸載")
...
def process_video(start_image_path, end_image_path=None, progress_callback=None, **kwargs):
    ...
    logger.info(f"[FramePack F1] total_second_length={params['total_second_length']}, total_latent_sections={total_latent_sections}")
    ...
    try:
        ...
        # 橋接片段（正序生成）：從「上一段」影片讀取「最後 19 幀」作為本段歷史參考，bridge 優先於 end
        bridge_history_latents = None  # 1,16,19,H/8,W/8
        prev_video_path = params.get('prev_video_path')
        if prev_video_path:
            try:
                import imageio.v2 as imageio
                progress.update(0, f'Loading previous segment (last 19 frames) from {os.path.basename(prev_video_path)} ...')
                ...
                if len(frames) >= 19:
                    ...
                    bridge_np = np.stack(processed, axis=0)  # 19,H,W,3
                    bridge_pt = torch.from_numpy(bridge_np).float() / 127.5 - 1.0
                    bridge_pt = bridge_pt.permute(0, 3, 1, 2).unsqueeze(0)  # 1,19,C,H,W
                    bridge_pt = bridge_pt.permute(0, 2, 1, 3, 4)  # 1,C,19,H,W
                    lat_list = []
                    for i in range(19):
                    ...
                    print(f"上一段影片最後 19 幀已載入，history latent 形狀: {bridge_history_latents.shape}")
                else:
                    print(f"警告: 上一段影片幀數不足 19，略過橋接。")
            except Exception as e:
                print(f"警告: 載入上一段影片橋接片段失敗 ({prev_video_path}): {e}")
                traceback.print_exc()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='FramePack F1 - 根據起始幀和結束幀生成影片')
